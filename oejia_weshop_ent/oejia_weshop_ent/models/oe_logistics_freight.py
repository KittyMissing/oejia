...
class LogisticsFreight(models.Model):
    ...
    transport_type = fields.Selection(defs.TransportType.attrs.items(), string='配送方式', default='express', required=True)
    unit = fields.Selection(defs.TransportationUnit.attrs.items(), string='单位', compute='_compute_unit')

    less_amount = fields.Float('数量内', required=True, default=0)
    less_price = fields.Float('数量内收费', required=True, default=0)
    increase_amount = fields.Float('每增加', required=True, default=0)
    increase_price = fields.Float('每增加加收费', required=True, default=0)

    district_transportation_ids = fields.One2many('oe.district.freight', 'default_transportation_id', string='区域运输费')

    _sql_constraints = [(
        'oe_transportation_logistics_transport_type_unique',
...
class DistrictFreight(models.Model):

    _name = 'oe.district.freight'
    _description = u'区域运输费'

    default_transportation_id = fields.Many2one('oe.logistics.freight', string='默认运输费', required=True, ondelete='cascade')

    transport_type = fields.Selection(defs.TransportType.attrs.items(), string='运输方式', compute='_compute_transport_type', store=True)
    unit = fields.Selection(defs.TransportationUnit.attrs.items(), string='单位', related='default_transportation_id.unit')
    ...
    less_amount = fields.Integer('数量内', required=True, default=0)
    less_price = fields.Float('数量内收费', required=True, default=0)
    increase_amount = fields.Integer('每增加', required=True, default=0)
    increase_price = fields.Float('每增加加收费', required=True, default=0)

    logistics_id = fields.Many2one('oe.logistics', string='物流', compute='_compute_logistics_id', store=True)
